...
import com.mcc.signsaya.feature.auth.repository.AuthErrorCause
...
class SignUpViewModel(
    ...
    fun onGoogleSignInStarted() {
        _state.update { it.copy(isGoogleSigningIn = true, banner = null) }
        bannerJob?.cancel()
    }
    ...
    fun onGoogleSignInError(message: String?) {
        _state.update {
            it.copy(
                isSubmitting = false,
                isGoogleSigningIn = false,
                banner = message?.let { msg -> Banner(msg, BannerType.ERROR) }
            )
        }
        if (message != null) startBannerTimer()
    }
