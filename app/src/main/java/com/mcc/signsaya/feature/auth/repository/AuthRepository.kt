package com.mcc.signsaya.feature.auth.repository
...
class AuthRepository(
    ...
    fun markVerificationDismissed() {
        val user = auth.currentUser ?: return
        prefs.edit { putBoolean("verification_dismissed_${user.uid}", true) }
    }

    fun isVerificationDismissed(): Boolean {
        val user = auth.currentUser ?: return false
        return prefs.getBoolean("verification_dismissed_${user.uid}", false)
    }

    fun setInitialEmailSent(sent: Boolean) {
        val user = auth.currentUser ?: return
        val now = if (sent) System.currentTimeMillis() else 0L
        prefs.edit {
            putBoolean("initial_email_sent_${user.uid}", sent)
            putLong("initial_email_sent_time_${user.uid}", now)
        }
    }

    fun isInitialEmailSent(): Boolean {
        val user = auth.currentUser ?: return false
        val sent = prefs.getBoolean("initial_email_sent_${user.uid}", false)
        if (!sent) return false
        
        val sentTime = prefs.getLong("initial_email_sent_time_${user.uid}", 0L)
        val threeDaysInMillis = 3 * 24 * 60 * 60 * 1000L
        
        return if (System.currentTimeMillis() - sentTime < threeDaysInMillis) {
            true
        } else {
            // Expired, clear it
